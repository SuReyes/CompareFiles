...
def _init_request(uri: str, headers: dict):
    ...
    full_uri = f"{base_url}{'' if base_url.endswith('/') else '/'}{'access-code-service'}{'' if uri.startswith('/') else ''}{uri}"
...
def put_request(uri: str, headers: dict, body: dict):
    full_uri, headers = _init_request(uri, headers)
    logger.info(f"Putting data to {full_uri} with headers {headers}")
    logger.info(json.dumps(body))
...
def patch_request(uri: str, headers: dict, body):
    full_uri, headers = _init_request(uri, headers)
